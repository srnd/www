export const getSupportedImages = function(callback) {
    ...
    if (typeof(window) === 'undefined') {
        callback({webp: true, jpx: false, jp2: false});
        return;
    }
    ...
    checkImage(WebP)
        ...
        .then((supportsJp2) => {
            const supports = {webp: false, jpx: false, jp2: false};
            if (supportsWebp) supports.webp = true;
            if (supportsJpx) supports.jpx = true;
            if (supportsJp2) supports.jp2 = true;
            callback(supports);
        });
}
